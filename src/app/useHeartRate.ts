...
// Set once by composeApp() as it builds the graph. Importing this module
// runs no side effects — nothing constructs a BleManager or any other
// native handle merely by being imported, unlike the old appStore.ts
...
/** Called by composeApp(); tests can bind a store composed from fakes. */
...
export function useHeartRate<T>(selector: (state: HeartRateState) => T): T {
  if (!store) throw new Error('useHeartRate called before composeApp()');
